...
var PROP_META = {
  ...
  'multi-family': { label:'Multi-Family',      color:'#0d9488', callout:null },
...
var CONTACT_COLORS = {
  ...
  'No Soliciting':'#92400e','No Access':'#78716c','Dog Present':'#ea580c',
  'Refused':'#c0392b',
...
function initFirebaseCanvasser() {
  ...
  // TEMPORARY DIAGNOSTIC — if loading takes too long, show which specific
  // step is stuck directly on screen (no DevTools needed). Remove once resolved.
  var rosterDone = false, daysDone = false;
  var diagTimer = setTimeout(function(){
    var loadingEl = document.getElementById('setup-loading');
    if (loadingEl) {
      loadingEl.textContent = 'Still loading — roster: ' + (rosterDone?'OK':'STUCK') + ', shifts: ' + (daysDone?'OK':'STUCK') + '. Waiting...';
    }
  }, 5000);

  var rosterP = loadRoster().then(function(r){ rosterDone = true; return r; });
  var daysP = loadCanvassDays().then(function(r){ daysDone = true; return r; });

  Promise.all([rosterP, daysP])
    .then(function(){
    ...
    .catch(function(e){
      clearTimeout(diagTimer);
      document.getElementById('setup-loading').textContent='Could not connect — check your internet connection. (' + (e && e.message ? e.message : 'unknown error') + ')';
    });
}
...
function startApp() {
  ...
  var defaultHood = (window.NEIGHBORHOODS&&window.NEIGHBORHOODS[0])?window.NEIGHBORHOODS[0].key:'';
  neighborhoodKeys = (shiftDay&&shiftDay.neighborhoods) || (shiftDay&&shiftDay.neighborhood?[shiftDay.neighborhood]:(defaultHood?[defaultHood]:[]));
  neighborhoodKey = neighborhoodKeys[0]||''; // kept for any code that only needs one representative hood
  ...
  var hoodNamesTop = neighborhoodKeys.map(function(k){
    var h=(window.NEIGHBORHOODS||[]).find(function(n){return n.key===k;});
    return h?h.displayName:k;
  }).join(' + ');
  document.getElementById('topbar-sub').textContent=hoodNamesTop+' · '+todayKey;
  // Build the address pool as the union of every neighborhood this shift
  // covers, tagging each address with which one it came from — needed to
  // split writes (logs, assignments) back to the correct per-neighborhood
  // Firebase path.
  addresses=[];
  neighborhoodKeys.forEach(function(hk){
    var hoodAddrs = window.NEIGHBORHOOD_DATA[hk]||[];
    hoodAddrs.forEach(function(a){
...
function buildCSV(){
  ...
  addresses.forEach(function(addr){
    ...
    var addrHood=(window.NEIGHBORHOODS.find(function(n){return n.key===addr._hoodKey;})||{}).displayName||addr._hoodKey||'';
    var dt=new Date(log.time);
    rows.push(['"'+addrHood+'"','"'+addr.n+' '+addr.s+'"','"'+addr.u+'"','"'+addr.s+'"','"'+((PROP_META[addr.t]||PROP_META.house).label)+'"',addr.lat,addr.lng,'"'+log.contact+'"',log.interest||'',log.litdrop?'Yes':'No',log.wantContact?'Yes':'No',log.wantSign?'Yes':'No','"'+(log.name||'').replace(/"/g,'""')+'"','"'+(log.party||'')+'"','"'+(log.phone||'').replace(/"/g,'""')+'"','"'+(log.notes||'').replace(/"/g,'""')+'"','"'+log.canvasser+'"','"'+dt.toLocaleDateString()+'"','"'+dt.toLocaleTimeString()+'"'].join(','));
  });
...
// ── FIREBASE CANVASSER ────────────────────
function pushLogs(){
  if(!fbConnected||!neighborhoodKeys.length) return;
  // logs can now contain addresses from more than one neighborhood — split
  // by each address's actual _hoodKey rather than writing everything to
  // one fixed path.
  var addrMap = {};
  ...
  if(signs.length) {
    ...
    signs.forEach(function(s){
      var hk = s.neighborhood || neighborhoodKey;
      if (!signsByHood[hk]) signsByHood[hk] = [];
      signsByHood[hk].push(s);
    });
    Object.keys(signsByHood).forEach(function(hk){
      fbPut('/'+hk+'/signs/'+todayKey, signsByHood[hk]).catch(function(){});
    });
  }
}
...
function pullCanvasserData(){
  if(!fbConnected||!neighborhoodKeys.length) return;
  // Pull assignments from EVERY neighborhood this shift covers and merge
  // — a route can now genuinely span more than one neighborhood.
  Promise.all(neighborhoodKeys.map(function(hk){
    return fbGet('/'+hk+'/assignments/'+todayKey).catch(function(){return null;});
  })).then(function(results){
    var combined = {}; // volId -> merged {route:[], assignments:{}}
    results.forEach(function(d){
      ...
      Object.entries(d).forEach(function(pair){
        ...
        var route = Array.isArray(data.route) ? data.route : Object.keys(assigns).filter(function(k){return k!=='route'&&k!=='assignments';});
        if(!combined[vid]) combined[vid] = {route:[], assignments:{}};
        var seen = {}; combined[vid].route.forEach(function(id){seen[id]=true;});
        route.forEach(function(id){ if(!seen[id]){seen[id]=true; combined[vid].route.push(id);} });
        Object.assign(combined[vid].assignments, assigns);
      });
    });
    ...
    myAssignments={};
    myRoute=[];
    Object.entries(combined).forEach(function(pair){
      var vid=pair[0], data=pair[1];
      data.route.forEach(function(addrId){
        addrToVolId[addrId]=vid;
        if(vid===myVolId){ myAssignments[addrId]=true; myRoute.push(addrId); }
      });
    });

    // Detect partner: another vol with substantial overlap with my route
    if (!partnerVolName) {
      Object.entries(combined).forEach(function(pair){
        var vid=pair[0], data=pair[1];
        if(vid===myVolId) return;
        var overlap=data.route.filter(function(id){return myAssignments[id];});
        if(overlap.length > 2) {
  ...
  // Pull today's logs from every neighborhood and merge
  Promise.all(neighborhoodKeys.map(function(hk){
    return fbGet('/'+hk+'/logs/'+todayKey).catch(function(){return null;});
  })).then(function(results){
    var changed=false;
    results.forEach(function(d){
      if(!d||typeof d!=='object') return;
...
// formatDate() lives in firebase.js — removed duplicate here to avoid the same shadowing risk we just fixed for fbFetch.
